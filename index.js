...
if (fs.existsSync(listenersPath)) {
  for (const file of fs.readdirSync(listenersPath)) {
    if (file.endsWith('.js')) {
      const mod = require(path.join(listenersPath, file));
...
async function start() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'open') 
      console.log(`Bot ${config.botName} online!`);
    ...
    if (connection === 'close') {
      ...
      else console.log('Logged out — hapus folder auth agar scan ulang');
    }
  });
